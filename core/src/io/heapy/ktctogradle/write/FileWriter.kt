...
internal class FileWriter(private val fileSystem: FileSystem) {
    ...
    fun write(
        ...
        val changed = files.filter { file ->
            !fileSystem.exists(file.path) || read(file.path) != file.content
        }
        val collisions = changed.filter { fileSystem.exists(it.path) && !isGenerated(read(it.path)) }
